...
percent_df<-cbind(identifier, ethnic, df1,df2,df3,df4,df5)%>%
    ...
    mutate_at(c(3), as.integer)%>%
    mutate_at(c(4:182), as.numeric)%>%
    mutate_at(c(4:182), ~(round(., 2)))%>%
    add_column(year = "2010-2011", .after = "name")
...
#subset dataframe to collect ethnic totals from total completions

totals_df<-data_df%>%select(c(ends_with('total_gran_tot')))%>%
    select(!c(unk_total_gran_tot, nonres_total_gran_tot))

# calculate diversity metrics for total completions

shannon_df<-data.frame(diversity(totals_df, index = "shannon", MARGIN = 1, base = exp(1)))%>%
    rename(inst_shannon_h = 'diversity.totals_df..index....shannon...MARGIN...1..base...exp.1..')%>%
    mutate(inst_shannon_e = inst_shannon_h/log(ncol(totals_df)))%>%
    mutate_if(is.numeric, round, digits = 2)

simpson_df<-data.frame(diversity(totals_df, index = "simpson", MARGIN = 1, base = exp(1)))%>%
    rename(inst_simpson = 'diversity.totals_df..index....simpson...MARGIN...1..base...exp.1..')%>%
    mutate_if(is.numeric, round, digits = 2)

inv_simpson_df<-data.frame(diversity(totals_df, index = "inv", MARGIN = 1, base = exp(1)))%>%
    rename(Inst_inv_simpson = 'diversity.totals_df..index....inv...MARGIN...1..base...exp.1..')%>%
    mutate_if(is.numeric, round, digits = 2)

instdiv_df<-cbind(identifier,shannon_df, simpson_df, inv_simpson_df)%>%
    mutate(across(.cols = everything(),~str_squish(.)))%>%
    add_column(year = "2010-2011", .after = "name")

#subset data for stem completions - drops to 433 institutions

stems_df<-data_df%>%select(c(1:2) | c(33:182))%>%
    drop_na()

tmp<-stems_df%>%select(!c(1:2))

# calculate diversity metrics across stem completion

shannon2_df<-data.frame(diversity(tmp, index = "shannon", MARGIN = 1, base = exp(1)))%>%
    rename(stem_shannon_h = 'diversity.tmp..index....shannon...MARGIN...1..base...exp.1..')%>%
    mutate(stem_shannon_e = stem_shannon_h/log(ncol(tmp)))%>%
    mutate_if(is.numeric, round, digits = 2)

simpson2_df<-data.frame(diversity(tmp, index = "simpson", MARGIN = 1, base = exp(1)))%>%
    rename(stem_simpson = 'diversity.tmp..index....simpson...MARGIN...1..base...exp.1..')%>%
    mutate_if(is.numeric, round, digits = 2)

inv_simpson2_df<-data.frame(diversity(tmp, index = "inv", MARGIN = 1, base = exp(1)))%>%
    rename(stem_inv_simpson = 'diversity.tmp..index....inv...MARGIN...1..base...exp.1..')%>%
    mutate_if(is.numeric, round, digits = 2)

stemsdiv_df<-cbind(shannon2_df, simpson2_df, inv_simpson2_df)%>%
    mutate(across(.cols = everything(),~str_squish(.)))%>%
    add_column(year = "2010-2011", .before = "stem_shannon_h")

stemsdiv_df<-cbind(stems_df[1:2], stemsdiv_df)
